...
def main():
    class App(ttk.Frame): # Make frame containing all input parameters
        def __init__(self, master):
            ...
            self.ac_drp["values"] = fn.get_aircraft()["aircraft"].tolist()
        ...
        def make_plot(self, save_name = None): # make a dataframe, call plotting function 
            out = fn.run_o10(aircraft = self.ac.get(), power = int(self.pwr.get()),
                       speed_kts = self.speed.get(), temp = self.temp.get(),
                       rel_hum_pct = self.rh_pct.get())
                       
            out_2 = fn.run_o10(aircraft = self.ac.get(), power = int(self.pwr_2.get()),
                       speed_kts = self.speed.get(), temp = self.temp.get(),
        ...
        def set_info(self, event): # Get units and engine of selected aircraft
            try:
                self.units.set(fn.get_info(self.ac.get(), df = fn.get_aircraft())["units"])
                self.eng.set(fn.get_info(self.ac.get(), df = fn.get_aircraft())["engine"])
            except:
                pass
